...
function getAircraftIconSvg(ac, color) {
    ...
    // 0. Business & Corporate Jets (Prioritized before commercial airliner checks so E55P, EMB5, Phenom, Citation, Gulfstream, Learjet get business jet icons)
        ...
        'HDJT','HONDAJET','SF50','VISION JET','EA50','ECLIPSE','SJ30'
    ]) || cat === 'business-jet') shapeKey = 'jet_swept';
    ...
    // 7. General Commercial Jet Category & Manufacturer String Search Fallback
        ...
        cat === 'commercial-jet' || cat === 'airline' || cat === 'heavy' ||
        ...
        (desc && (desc.includes('BOEING') || desc.includes('AIRBUS') || desc.includes('AIRLINER')))
    ) {
        shapeKey = 'jet_swept';
    }
            ...
            const code = `javascript:(function(){
                ...
                function s(){
                    try {
                        ...
                        console.log("[Sync] Scanning " + txt.length + " characters...");
                        ...
                        if (latMatch && lonMatch) {
                            ...
                            fetch(url, {
                                method:'POST',
                                headers:{'Content-Type':'application/json'},
                                body:JSON.stringify({ tail: tail, lat: lat, lon: lon, alt: parseInt(altMatch[1]), speed: parseInt(spdMatch[1]), heading: parseInt(hdgMatch[1]) })
                            }).then(function(r){ console.log("[Sync] Push success:", r.status); })
                              .catch(function(e){ console.warn('[Sync] Push error:', e); showErr('Network push failed.'); });
                        } else {
                            console.warn("[Sync] Could not find Lat/Lon!");
